...
const Header = (props: HeaderProps): any => {
	...
	const ChangePasswordButton = withRouter(({ history }: any) =>
		Boolean(user) ? <Button onClick={() => history.push('/ChangePassword')}>Change Password</Button> : <div />
	);

	const showLogoutButton = (): any => {
		const buttonRender = Boolean(user) ? (
			<Button id="logoutButton" onClick={handleLogout} color="inherit">
				Logout
			</Button>
		) : (
			EMPTY
