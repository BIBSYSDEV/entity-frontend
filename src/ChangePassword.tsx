...
const ChangePassword = (props: ChangePasswordProps): any => {
	...
	const SubmitButton = withRouter(({ history }: any) =>
		Boolean(user) ? (
			<Button
				type="submit"
				fullWidth
				...
				onClick={(event: any): Promise<any> => handleSubmit(event, history)}
			>
				Change Password
			</Button>
		) : (
	...
	const CancelButton = withRouter(({ history }: any) =>
		Boolean(user) ? (
			<Button
				fullWidth
				variant="contained"
			...
			<Container component="main" maxWidth="xs">
				...
				<div>
					<Typography variant="subtitle2">
						(Password must have a minimum of 12 characters, upper and lower case and at
						least 1 number)
					</Typography>
					<form noValidate>
						<TextField
